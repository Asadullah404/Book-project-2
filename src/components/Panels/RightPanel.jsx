...
export default function RightPanel() {
    ...
    useEffect(() => {
        const fetchCodeMap = async () => {
            try {
                const res = await fetch('/content/code_map.json');
                if (res.ok) {
                    const map = await res.json();
                    setCodeMap(map);
                }
            } catch (err) {
                console.error("Failed to load code map", err);
            }
        };
        fetchCodeMap();
    }, []);

    // Update code snippet based on chapter
    useEffect(() => {
        if (currentChapter) {
            if (codeMap[currentChapter.id]) {
                setCode(codeMap[currentChapter.id]);
            } else if (currentChapter.id === 'chap2') {
                setCode('# Example: Variables\nx = 10\ny = 20\nprint(f"Sum: {x + y}")');
            }
        }
    }, [currentChapter, codeMap]);
